...
"""
        ...
        # Cap the repair call at 15s. Without this, a slow Gemini response would
        # stack on top of the primary LLM call and push the request past the
        # 45s server timeout, producing a 503 instead of a degraded-but-shown
        # prose response. 15s is empirically sufficient for Gemini 2.5 Flash
        # to return a structured recipe when it's going to succeed.
        repair_response = await asyncio.wait_for(llm.ainvoke(repair_messages), timeout=15.0)
        ...
        logger.warning("healthify.repair.timeout user_input=%r", user_input[:80])
        return None
    except Exception as exc:
        ...
        if stream:
            async def _fridge_stream():
                ...
                try:
                    ...
                    if not payload.get("recipe"):
                        repaired = await repair_missing_recipe(user_input, user_context)
                        if repaired and repaired.get("recipe"):
                            payload["recipe"] = repaired["recipe"]
                            payload["message"] = repaired.get("message") or payload.get("message")
                            payload["swaps"] = repaired.get("swaps") or payload.get("swaps")
                    mes = _compute_recipe_mes_from_cache(cached, payload.get("nutrition"))
                    if mes:
